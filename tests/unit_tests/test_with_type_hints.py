from word_counter.with_type_hints import CountResults, Counts, word_count


def test_total_counts_matches_unique_counts():
    """Total and unique counts should match when all words and chars are unique"""
    # setup
    expected = CountResults(
        words=Counts(total=2, unique=2),
        chars=Counts(total=7, unique=7),
    )
    # execution
...
def test_total_counts_different_from_unique_counts():
    ...
    # setup
    expected = CountResults(
        words=Counts(total=2, unique=1),
        chars=Counts(total=7, unique=4),
    )
    # execution
